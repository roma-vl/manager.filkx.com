...
<template>
    <div class="flex gap-6 overflow-x-auto pb-4">
        <div v-for="status in statuses" :key="status.id" class="w-80 shrink-0 bg-gray-900 rounded-lg shadow p-4">
            <h2 class="text-sm font-bold text-white uppercase mb-3 tracking-wide">
                {{ status.name }}
            </h2>

            <div v-for="task in tasksByStatus(status.id)" :key="task.id" class="bg-gray-800 hover:bg-gray-700 transition rounded-lg p-4 mb-3">
