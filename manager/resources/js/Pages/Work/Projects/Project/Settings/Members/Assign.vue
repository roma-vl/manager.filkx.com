            ...
            <div class="card-body space-y-6">
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-1">
                        <label v-for="(name, id) in departments" :key="id" class="inline-flex items-center gap-2">
                            <input type="checkbox" :value="id" v-model="form.departments" />
                            {{ name }}
                        </label>
                    </div>
                    <div class="text-red-600 text-sm" v-if="form.errors.departments">{{ form.errors.departments }}</div>
                </div>
                ...
                <div v-if="form.errors.message" class="text-red-700 font-medium">{{ form.errors.message }}</div>
            </div>
        </form>
    </AppLayout>
</template>
