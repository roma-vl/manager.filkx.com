...
function truncateFileName(filename, maxBaseLength = 20) {
    ...
    const ext = parts.pop(); // розширення
    const base = parts.join('.'); // базова назва

    const shortBase = base.length > maxBaseLength
    ...
    <div class="mt-6">
        ...
        <div v-if="localFiles?.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            >
                ...
                <!-- Top Overlay -->
                <div class="relative z-10 p-2 flex justify-between items-start space-x-2 bg-white/80 dark:bg-gray-900/70 backdrop-blur-sm">
                    <a
                        :href="fileUrl(file)"
                        target="_blank"
                ...
                >
                    <div>
                        👤
                        <Link
                            :href="`/work/members/${file.member.id}`"
