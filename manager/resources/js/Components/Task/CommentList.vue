...
function formatDate(dateStr) {
    return new Date(dateStr).toLocaleString('uk-UA', { hour12: false })
}
</script>
        ...
        >
            ...
            />
                        <div class="mt-2 flex space-x-2">
                            <button @click="saveEdit(comment.id)" class="bg-green-600 text-white px-4 py-1 rounded-xl">Змінити</button>
                            <button @click="cancelEdit" class="bg-gray-300 text-gray-800 px-4 py-1 rounded-xl">Скасувати</button>
                        </div>
                    </div>
                    <div v-else>
                ...
                <div class="flex flex-col space-y-1 ml-4">
                    <button v-if="editingId !== comment.id" @click="startEditComment(comment)" class="text-xs text-indigo-600">✎ Редагувати</button>
                    <button @click="deleteComment(comment.id)" class="text-xs text-red-500">✖ Видалити</button>
                </div>
            </div>
        </div>
    </div>
