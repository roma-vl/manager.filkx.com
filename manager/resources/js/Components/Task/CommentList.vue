...
<template>
    <div class="dark:bg-[#0e0f11] bg-white dark:text-white text-gray-900 mt-6
              rounded-2xl shadow-xl transition-all duration-300 ease-in-out max-w-4xl mx-auto p-4">
        <h2 class="text-xl font-bold mb-4">Коментарі</h2>

        <div v-if="comments.length === 0" class="text-gray-400 dark:text-gray-500 mb-4">Ще немає коментарів.</div>
        ...
        <div v-for="comment in comments" :key="comment.id"
            ...
            <div class="flex justify-between items-start">
                ...
                <textarea v-model="editForm.text"
                          class="w-full rounded-xl p-2 text-sm dark:bg-indigo-950 bg-white
                                 text-gray-900 dark:text-white border focus:ring focus:ring-indigo-500"
                          rows="4"
                          required />
                    ...
                    <div v-else>
                        ...
                        <div class="text-xs text-gray-500 mt-2">
                            Автор: <span class="font-semibold">{{ comment.author_name }}</span> • {{ formatDate(comment.date) }}
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col space-y-1 ml-4">
                    <button v-if="editingId !== comment.id" @click="startEditComment(comment)"
      ...
      <textarea
          ...
          class="w-full border dark:border-gray-700 rounded-xl p-3 text-sm
               dark:bg-indigo-950 bg-white text-gray-900 dark:text-white focus:outline-none focus:ring-2
      ...
      ></textarea>
            ...
            <button
                ...
                class="mt-3 px-5 py-2 bg-indigo-600 text-white rounded-xl
