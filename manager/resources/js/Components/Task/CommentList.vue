    ...
    >
      <div class="flex justify-between items-start">
        <div class="flex-1">
          <div v-if="editingId === comment.id">
            ...
            <div class="mt-2 flex space-x-2">
              <button
                class="bg-green-600 text-white px-4 py-1 rounded-xl"
                @click="saveEdit(comment.id)"
              >
                Змінити
              </button>
              <button class="bg-gray-300 text-gray-800 px-4 py-1 rounded-xl" @click="cancelEdit">
                Скасувати
              </button>
            </div>
          </div>
          <div v-else>
            ...
            <div class="text-xs text-gray-500 mt-2">
              ...
              {{ formatUtcDate(comment.date.date) }}
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col space-y-1 ml-4">
          ...
          <button class="text-xs text-red-500" @click="deleteComment(comment.id)">
            ✖ Видалити
          </button>
        </div>
      </div>
